...
#pragma pack(pop)
	...
	task = convert_port_to_map_read(In0P->Head.msgh_request_port);
...
mig_internal kern_return_t __MIG_check__Request__vm_purgable_control_external_t(__attribute__((__unused__)) __Request__vm_purgable_control_external_t *In0P)
{
...
#if	defined(__MIG_check__Request__vm_purgable_control_external_t__defined)
	check_result = __MIG_check__Request__vm_purgable_control_external_t((__Request *)In0P);
	if (check_result != MACH_MSG_SUCCESS)
		{ MIG_RETURN_ERROR(OutP, check_result); }
#endif	/* defined(__MIG_check__Request__vm_purgable_control_external_t__defined) */

	OutP->RetCode = vm_purgable_control_external(In0P->Head.msgh_request_port, In0P->address, In0P->control, &In0P->state);
	if (OutP->RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, OutP->RetCode);
	}
#if	__MigKernelSpecificCode
...
const struct vm_map_subsystem vm_map_subsystem = {
	...
	(vm_address_t)0,
	{
		...
		{0, 0, 0, 0, 0, 0},
          { (mig_impl_routine_t) 0,
          (mig_stub_routine_t) _Xvm_purgable_control_external, 5, 0, (routine_arg_descriptor_t)0, (mach_msg_size_t)sizeof(__Reply__vm_purgable_control_external_t)},
          { (mig_impl_routine_t) 0,
