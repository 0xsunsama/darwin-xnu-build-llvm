...
mig_internal kern_return_t __MIG_check__Request__host_get_io_main_t(__attribute__((__unused__)) __Request__host_get_io_main_t *In0P)
{
...
#if	defined(__MIG_check__Request__host_get_io_main_t__defined)
	check_result = __MIG_check__Request__host_get_io_main_t((__Request *)In0P);
	if (check_result != MACH_MSG_SUCCESS)
		{ MIG_RETURN_ERROR(OutP, check_result); }
#endif	/* defined(__MIG_check__Request__host_get_io_main_t__defined) */

#if	UseStaticTemplates
	OutP->io_main = io_mainTemplate;
#else	/* UseStaticTemplates */
#if __MigKernelSpecificCode
	OutP->io_main.disposition = 17;
#else
	OutP->io_main.disposition = 19;
#endif /* __MigKernelSpecificCode */
#if !(defined(KERNEL) && defined(__LP64__))
	OutP->io_main.pad1 = 0;
#endif
	OutP->io_main.pad2 = 0;
	OutP->io_main.type = MACH_MSG_PORT_DESCRIPTOR;
#if defined(KERNEL)
	OutP->io_main.pad_end = 0;
#endif
#endif	/* UseStaticTemplates */


	RetCode = host_get_io_main(convert_port_to_host(In0P->Head.msgh_request_port), &OutP->io_main.name);
	if (RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, RetCode);
	}
#if	__MigKernelSpecificCode
...
#pragma pack(pop)
	...
	RetCode = mach_memory_info(In0P->Head.msgh_request_port, (mach_zone_name_array_t *)&(OutP->names.address), &OutP->namesCnt, (mach_zone_info_array_t *)&(OutP->info.address), &OutP->infoCnt, (mach_memory_info_array_t *)&(OutP->memory_info.address), &OutP->memory_infoCnt);
	if (RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, RetCode);
	}
#if	__MigKernelSpecificCode
...
const struct mach_host_subsystem mach_host_subsystem = {
	...
	(vm_address_t)0,
	{
          ...
          { (mig_impl_routine_t) 0,
          (mig_stub_routine_t) _Xhost_get_io_main, 2, 0, (routine_arg_descriptor_t)0, (mach_msg_size_t)sizeof(__Reply__host_get_io_main_t)},
          { (mig_impl_routine_t) 0,
