...
choose_xnu() {
	if [ -z "$MACOS_VERSION"]; then
		...
		MACOS_VERSION=$(gum choose "13.0" "13.1" "13.2" "13.3" "13.4" "13.5" "14.0" "14.1" "14.2")
	fi
...
patches_14() {
	...
	directory="./fakeroot/usr/local/include/firehose"
	...
	cp -r ./xnu/libkern/os ./fakeroot/usr/local/include/firehose
	cp -r ./xnu/osfmk/machine ./fakeroot/usr/local/include/firehose
	cp ./xnu/libkern/firehose/*.h ./fakeroot/usr/local/include/firehose
	cp -r ./xnu/osfmk/atm ./fakeroot/usr/local/include/firehose

	directory="./xnu"

	# 遍历目录下的所有Makefile文件
	for file in $(find "$directory" -name "Makefile"); do
		# 在文件中替换"-mach_msg2"为空字符串，并将结果保存到临时文件
		sed -e 's/-mach_msg2//g' "$file" >"$file.tmp"
		# 将临时文件替换为原始文件
		mv "$file.tmp" "$file"
		echo "Modified Makefile: $file"
	done
...
build_xnu() {
	cp -r ./TrustCache ./xnu/EXTERNAL_HEADERS/
