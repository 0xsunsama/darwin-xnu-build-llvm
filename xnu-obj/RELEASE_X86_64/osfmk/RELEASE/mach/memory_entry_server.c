...
mig_internal kern_return_t __MIG_check__Request__mach_memory_entry_ownership_from_user_t(__attribute__((__unused__)) __Request__mach_memory_entry_ownership_from_user_t *In0P)
{
...
#if	defined(__MIG_check__Request__mach_memory_entry_ownership_from_user_t__defined)
	check_result = __MIG_check__Request__mach_memory_entry_ownership_from_user_t((__Request *)In0P);
	if (check_result != MACH_MSG_SUCCESS)
		{ MIG_RETURN_ERROR(OutP, check_result); }
#endif	/* defined(__MIG_check__Request__mach_memory_entry_ownership_from_user_t__defined) */

	OutP->RetCode = mach_memory_entry_ownership_from_user(null_conversion(In0P->Head.msgh_request_port), In0P->owner.name, In0P->ledger_tag, In0P->ledger_flags);
#if	__MigKernelSpecificCode
#endif /* __MigKernelSpecificCode */
...
const struct memory_entry_subsystem memory_entry_subsystem = {
	...
	(vm_address_t)0,
	{
          ...
          { (mig_impl_routine_t) 0,
          (mig_stub_routine_t) _Xmach_memory_entry_ownership_from_user, 4, 0, (routine_arg_descriptor_t)0, (mach_msg_size_t)sizeof(__Reply__mach_memory_entry_ownership_from_user_t)},
	}
};
