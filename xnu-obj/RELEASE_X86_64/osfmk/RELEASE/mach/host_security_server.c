...
#pragma pack(pop)
	...
	parent_task = convert_port_to_task_mig(In0P->parent_task.name);

	RetCode = host_security_create_task_token(In0P->Head.msgh_request_port, parent_task, In0P->sec_token, In0P->audit_token, convert_port_to_host(In0P->host.name), (ledger_array_t)(In0P->ledgers.address), In0P->ledgers.count, In0P->inherit_memory, &child_task);
...
#endif /* __MigKernelSpecificCode */
	OutP->child_task.name = (mach_port_t)convert_task_to_port_kernel(child_task);
...
#pragma pack(pop)
	...
	target_task = convert_port_to_task_mig(In0P->target_task.name);

	OutP->RetCode = host_security_set_task_token(In0P->Head.msgh_request_port, target_task, In0P->sec_token, In0P->audit_token, convert_port_to_host(In0P->host.name));
