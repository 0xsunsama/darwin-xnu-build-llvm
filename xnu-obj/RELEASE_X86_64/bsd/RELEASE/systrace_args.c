...
systrace_args(int sysnum, void *params, uint64_t *uarg)
{
	...
	/* sys_crossarch_trap */
	case 38: {
		struct crossarch_trap_args *p = params;
		uarg[0] = (uint64_t) p->name; /* uint32_t */
		break;
	}
	/* getppid */
	case 39: {
	...
	/* sys_panic_with_data */
	case 185: {
		struct panic_with_data_args *p = params;
		uarg[0] = (uint64_t) p->uuid; /* uuid_t */
		uarg[1] = (uint64_t) p->addr; /* void * */
		uarg[2] = (uint64_t) p->len; /* uint32_t */
		uarg[3] = (uint64_t) p->flags; /* uint32_t */
		uarg[4] = (uint64_t) p->msg; /* const char * */
		break;
	}
	/* thread_selfcounts */
	case 186: {
		struct thread_selfcounts_args *p = params;
		uarg[0] = (uint64_t) p->kind; /* uint32_t */
		uarg[1] = (uint64_t) p->buf; /* user_addr_t */
		uarg[2] = (uint64_t) p->size; /* user_size_t */
		break;
	}
	/* fdatasync */
	case 187: {
	...
	case 218: {
		struct openat_dprotected_np_args *p = params;
		iarg[0] = (int64_t) p->fd; /* int */
		uarg[1] = (uint64_t) p->path; /* user_addr_t */
		iarg[2] = (int64_t) p->flags; /* int */
		iarg[3] = (int64_t) p->class; /* int */
		iarg[4] = (int64_t) p->dpflags; /* int */
		iarg[5] = (int64_t) p->mode; /* int */
		iarg[6] = (int64_t) p->authfd; /* int */
		break;
	}
	/* getattrlist */
	case 220: {
	...
	case 361: {
		...
		uarg[3] = (uint64_t) p->sema_or_ulock; /* user_addr_t */
		break;
	}
	/* kqueue */
	case 362: {
	...
	/* __nexus_open */
	case 503: {
		struct __nexus_open_args *p = params;
		uarg[0] = (uint64_t) p->init; /* struct nxctl_init * */
		uarg[1] = (uint64_t) p->init_len; /* uint32_t */
		break;
	}
	/* __nexus_register */
	case 504: {
		struct __nexus_register_args *p = params;
		iarg[0] = (int64_t) p->ctl; /* int */
		uarg[1] = (uint64_t) p->reg; /* struct nxprov_reg * */
		uarg[2] = (uint64_t) p->reg_len; /* uint32_t */
		uarg[3] = (uint64_t) p->prov_uuid; /* uuid_t * */
		uarg[4] = (uint64_t) p->prov_uuid_len; /* uint32_t */
		break;
	}
	/* __nexus_deregister */
	case 505: {
		struct __nexus_deregister_args *p = params;
		iarg[0] = (int64_t) p->ctl; /* int */
		uarg[1] = (uint64_t) p->prov_uuid; /* uuid_t */
		uarg[2] = (uint64_t) p->prov_uuid_len; /* uint32_t */
		break;
	}
	/* __nexus_create */
	case 506: {
		struct __nexus_create_args *p = params;
		iarg[0] = (int64_t) p->ctl; /* int */
		uarg[1] = (uint64_t) p->prov_uuid; /* uuid_t */
		uarg[2] = (uint64_t) p->prov_uuid_len; /* uint32_t */
		uarg[3] = (uint64_t) p->nx_uuid; /* uuid_t * */
		uarg[4] = (uint64_t) p->nx_uuid_len; /* uint32_t */
		break;
	}
	/* __nexus_destroy */
	case 507: {
		struct __nexus_destroy_args *p = params;
		iarg[0] = (int64_t) p->ctl; /* int */
		uarg[1] = (uint64_t) p->nx_uuid; /* uuid_t */
		uarg[2] = (uint64_t) p->nx_uuid_len; /* uint32_t */
		break;
	}
	/* __nexus_get_opt */
	case 508: {
		struct __nexus_get_opt_args *p = params;
		iarg[0] = (int64_t) p->ctl; /* int */
		uarg[1] = (uint64_t) p->opt; /* uint32_t */
		uarg[2] = (uint64_t) p->aoptval; /* void * */
		uarg[3] = (uint64_t) p->aoptlen; /* uint32_t * */
		break;
	}
	/* __nexus_set_opt */
	case 509: {
		struct __nexus_set_opt_args *p = params;
		iarg[0] = (int64_t) p->ctl; /* int */
		uarg[1] = (uint64_t) p->opt; /* uint32_t */
		uarg[2] = (uint64_t) p->aoptval; /* const void * */
		uarg[3] = (uint64_t) p->optlen; /* uint32_t */
		break;
	}
	/* __channel_open */
	case 510: {
		struct __channel_open_args *p = params;
		uarg[0] = (uint64_t) p->init; /* struct ch_init * */
		uarg[1] = (uint64_t) p->init_len; /* uint32_t */
		break;
	}
	/* __channel_get_info */
	case 511: {
		struct __channel_get_info_args *p = params;
		iarg[0] = (int64_t) p->c; /* int */
		uarg[1] = (uint64_t) p->cinfo; /* struct ch_info * */
		uarg[2] = (uint64_t) p->cinfolen; /* uint32_t */
		break;
	}
	/* __channel_sync */
	case 512: {
		struct __channel_sync_args *p = params;
		iarg[0] = (int64_t) p->c; /* int */
		iarg[1] = (int64_t) p->mode; /* int */
		iarg[2] = (int64_t) p->flags; /* int */
		break;
	...
	/* __channel_get_opt */
	case 513: {
		struct __channel_get_opt_args *p = params;
		iarg[0] = (int64_t) p->c; /* int */
		uarg[1] = (uint64_t) p->opt; /* uint32_t */
		uarg[2] = (uint64_t) p->aoptval; /* void * */
		uarg[3] = (uint64_t) p->aoptlen; /* uint32_t * */
	...
	/* __channel_set_opt */
	case 514: {
		struct __channel_set_opt_args *p = params;
		iarg[0] = (int64_t) p->c; /* int */
		uarg[1] = (uint64_t) p->opt; /* uint32_t */
		uarg[2] = (uint64_t) p->aoptval; /* const void * */
		uarg[3] = (uint64_t) p->optlen; /* uint32_t */
	...
	/* tracker_action */
	case 546: {
		struct tracker_action_args *p = params;
		iarg[0] = (int64_t) p->action; /* int */
		uarg[1] = (uint64_t) p->buffer; /* char * */
		uarg[2] = (uint64_t) p->buffer_size; /* size_t */
		break;
	}
#else
#endif /* SOCKETS */
	...
	case 547: {
		struct debug_syscall_reject_args *p = params;
		uarg[0] = (uint64_t) p->packed_selectors; /* uint64_t */
		break;
	}
	/* sys_debug_syscall_reject_config */
	case 548: {
		struct debug_syscall_reject_config_args *p = params;
		uarg[0] = (uint64_t) p->packed_selectors1; /* uint64_t */
		uarg[1] = (uint64_t) p->packed_selectors2; /* uint64_t */
		uarg[2] = (uint64_t) p->flags; /* uint64_t */
		break;
	}
	/* graftdmg */
	case 549: {
		struct graftdmg_args *p = params;
		iarg[0] = (int64_t) p->dmg_fd; /* int */
		uarg[1] = (uint64_t) p->mountdir; /* const char * */
		uarg[2] = (uint64_t) p->graft_type; /* uint32_t */
		uarg[3] = (uint64_t) p->gda; /* graftdmg_args_un * */
		break;
	}
	/* map_with_linking_np */
	case 550: {
		struct map_with_linking_np_args *p = params;
		uarg[0] = (uint64_t) p->regions; /* void * */
		uarg[1] = (uint64_t) p->region_count; /* uint32_t */
		uarg[2] = (uint64_t) p->link_info; /* void * */
		uarg[3] = (uint64_t) p->link_info_size; /* uint32_t */
		break;
	}
	/* freadlink */
	case 551: {
		struct freadlink_args *p = params;
		iarg[0] = (int64_t) p->fd; /* int */
		uarg[1] = (uint64_t) p->buf; /* user_addr_t */
		uarg[2] = (uint64_t) p->bufsize; /* user_size_t */
		break;
	...
	case 552: {
		struct record_system_event_args *p = params;
		uarg[0] = (uint64_t) p->type; /* uint32_t */
		uarg[1] = (uint64_t) p->subsystem; /* uint32_t */
		uarg[2] = (uint64_t) p->event; /* const char * */
		uarg[3] = (uint64_t) p->payload; /* const char * */
		break;
	}
	/* mkfifoat */
	case 553: {
	...
	case 554: {
		...
		uarg[1] = (uint64_t) p->path; /* user_addr_t */
		iarg[2] = (int64_t) p->mode; /* int */
		iarg[3] = (int64_t) p->dev; /* int */
		break;
	}
	/* ungraftdmg */
	case 555: {
		struct ungraftdmg_args *p = params;
		uarg[0] = (uint64_t) p->mountdir; /* const char * */
		uarg[1] = (uint64_t) p->flags; /* uint64_t */
		break;
	}
	default:
