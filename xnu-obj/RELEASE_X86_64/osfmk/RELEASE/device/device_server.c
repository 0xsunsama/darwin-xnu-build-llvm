...
#pragma pack(pop)
	...
	into_task = convert_port_to_task_mig(In0P->into_task.name);
...
#pragma pack(pop)
	...
	owningTask = convert_port_to_task_mig(In0P->owningTask.name);
...
#pragma pack(pop)
	...
	from_task = convert_port_to_task_mig(In0P->from_task.name);
...
mig_internal kern_return_t __MIG_check__Request__io_service_wait_quiet_with_options_t(__attribute__((__unused__)) __Request__io_service_wait_quiet_with_options_t *In0P)
{
...
#pragma pack(pop)
...
#if	defined(__MIG_check__Request__io_service_wait_quiet_with_options_t__defined)
	check_result = __MIG_check__Request__io_service_wait_quiet_with_options_t((__Request *)In0P);
	if (check_result != MACH_MSG_SUCCESS)
		{ MIG_RETURN_ERROR(OutP, check_result); }
#endif	/* defined(__MIG_check__Request__io_service_wait_quiet_with_options_t__defined) */
	...
	OutP->RetCode = is_io_service_wait_quiet_with_options(service, In0P->wait_time, In0P->options);
	iokit_remove_reference(service);
#if	__MigKernelSpecificCode
#endif /* __MigKernelSpecificCode */
...
const struct is_iokit_subsystem is_iokit_subsystem = {
	...
	(vm_address_t)0,
	{
		...
		{0, 0, 0, 0, 0, 0},
          ...
          { (mig_impl_routine_t) 0,
          (mig_stub_routine_t) _Xio_service_wait_quiet_with_options, 4, 0, (routine_arg_descriptor_t)0, (mach_msg_size_t)sizeof(__Reply__io_service_wait_quiet_with_options_t)},
	}
};
